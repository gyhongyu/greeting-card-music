...
6. **死因六：在播放端或工坊端擅自拿本地寫死常數取代/覆蓋 GAS 雲端即時參數（致命大坑）**
   - 💥 後果：使用者在工坊編輯了新特效（如將中秋卡改為「天上掉月餅」）並發布至雲端，但播放器代理人自作聰明拿本地舊常數（金桂玉兔）去直接取代，導致受眾端永遠看不到定製效果。
   - 🛡️ 防線：**GAS 雲端數據為絕對唯一真理源 (SSOT)**！播放器一律以 GAS 返回之最新數據為準，嚴禁本地常數搶佔覆蓋！本地常數僅能作為斷網無法連線時的最後安全降級。
