...
- [x] 排版文字回放解鎖：移除 CSS 懸停/點擊暫停鎖死機制；拔除全局 !important 解鎖字級與寬度即時縮放。
- [x] 社群 OG 標籤與開場加載閘門：index.html 補齊 Open Graph / Twitter 預覽標籤；受眾端播放鍵轉圈圈並行拉取 1 卡 + 1 模 SSOT 雲端同步，移除所有超時降等邏輯。
- [x] 全域快取破除與純淨分享防截斷：注入 ?v=2/?v=3 破除瀏覽器圖標與 JS 頑固快取；收件人姓名純淨化並加入標點/空格防呆清洗，雙換行徹底根除社群截斷。
